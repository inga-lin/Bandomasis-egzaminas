...
app.get("/", (req, res) => {
  ...
  app.get('/admin/praso-manager', (req, res) => {//900//1000 dadejau dj.size,  LEFT JOIN dydziai as dj ON m.dydziai_id = dj.id
    // SELECT column1, column2, ...
    // FROM table_name; FROM (trees- duomenu bazes pavadinimas)
  ...
  app.delete('/praso-manager/:id', (req, res) => { //delytinam is trees lnteles kurio id yra ?(kazkoks)
      const sql = `
          DELETE FROM praso
          WHERE id = ?
          `;
  ...
  app.get("/praso-list-sorted/", (req, res) => {
    
    let sql;
  
    if (req.query.by == 'title' && req.query.dir == 'asc'){
      sql = `SELECT * FROM praso ORDER BY name ASC`;
    }
    else if (req.query.by == 'title' && req.query.dir == 'desc'){
      sql = `SELECT * FROM praso ORDER BY name DESC`;
    }
    else if (req.query.by == 'height' && req.query.dir == 'asc'){
      sql = `SELECT * FROM praso ORDER BY height ASC`;
    }
    else{
      sql = `SELECT * FROM praso ORDER BY height DESC`;
    }
    con.query(
      sql,
      (err, results) => {
  ...
  app.get("/praso-list-search", (req, res) => {
    const sql = `
          SELECT
          *
          FROM praso
  ...
  //300 302 vote +
  app.post("/praso-vote/:id", (req, res) => {
    const sql = `
          UPDATE praso
  ...
  app.delete("/praso-delete-comment/:id", (req, res) => { //delytinam is trees lnteles kurio id yra ?(kazkoks)
    const sql = `
  ...
  //medziu dyddziai
  app.get("/admin/praso-sizes", (req, res) => {///900 pasidejau cia admin
    const sql = `
    SELECT
    *
    FROM dydziai
  ...
  //1000 istrinti size 
  app.delete("/praso-manager-sizes/:id", (req, res) => {
    const sql = `
          DELETE FROM dydziai
          WHERE id = ?
          `;
